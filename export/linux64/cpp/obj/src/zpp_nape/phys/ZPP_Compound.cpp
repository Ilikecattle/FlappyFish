...
bool ZPP_Compound_obj::compounds_adder( ::nape::phys::Compound x){
	...
	if (((cur == x->zpp_inner))){
		HX_STACK_LINE(285)
		::String _g = ((HX_CSTRING("Error: Assignment would cause a cycle in the Compound tree: assigning ") + x->toString()) + HX_CSTRING(".compound = "));		HX_STACK_VAR(_g,"_g");
		HX_STACK_LINE(285)
		HX_STACK_DO_THROW((_g + this->outer->toString()));
		HX_STACK_LINE(286)
		return false;
	}
	HX_STACK_LINE(289)
